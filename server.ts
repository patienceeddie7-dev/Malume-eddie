...
async function startServer() {
  ...
  // AI Credit Risk Assessment Proxy
...
3. Final underwriting recommendation for CASH FIRST GROUP loan officers.`;
      ...
      // High-quality fallback response when Gemini key is not configured or empty
  ...
  // AI Support Chat Assistant Proxy
  app.post('/api/ai-chat', async (req, res) => {
    try {
      ...
      const ai = getGenAI();
      if (ai) {
        // Build conversational context if history provided
...
- Tone: Welcoming, courteous, transparent, clear, and reassuring. Keep answers concise (2 to 4 sentences) unless detailed steps are requested.`,
      ...
      // Intelligent rule-based fallback response if Gemini key is pending
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`CashFirst Finance server running on http://0.0.0.0:${PORT}`);
  });
}
